 ...
 */
function buildTreeFromPaths(tree: FileNode, pathSegments: string[]) {
  ...
  if (correspondingNode.length === 0) {
    ...
    const newNode: FileNode = {
      name: pathSegments[0],
      url: `${tree.url}/${pathSegments[0]}`,
      children: [],
    };
    tree.children.push(newNode);
    buildTreeFromPaths(newNode, pathSegments.slice(1)); // Recursively fill the new node with remaining path segments.
  } else {
    buildTreeFromPaths(correspondingNode[0], pathSegments.slice(1));
  }
...
function constructFileTree(paths: string[]): FileNode {
  ...
  paths.forEach(path => buildTreeFromPaths(tree, path.split(/[\\/]/))); // Build the tree by splitting each path into path segments and filling the tree.
